...
namespace CityTraveler.Infrastucture.Data
{
    public class ApplicationContext : IdentityDbContext<ApplicationUserModel, ApplicationUserRole, Guid>, IDbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            //not added
            builder.Entity<EventModel>().HasOne(x => x.Address).WithOne(x => x.EventModel).HasForeignKey<EntertaimentAddressModel>(x => x.Id).OnDelete(DeleteBehavior.Cascade);
            builder.Entity<LandskapeModel>().HasOne(x => x.Address).WithOne(x => x.LandskapeModel).HasForeignKey<EntertaimentAddressModel>(x => x.Id).OnDelete(DeleteBehavior.Cascade);
            builder.Entity<InstitutionModel>().HasOne(x => x.Address).WithOne(x => x.InstitutionMode).HasForeignKey<EntertaimentAddressModel>(x => x.Id).OnDelete(DeleteBehavior.Cascade);

            builder.Entity<EventModel>().HasMany(x => x.Reviews).WithOne(x => x.EventModel).HasForeignKey(x => x.Id).OnDelete(DeleteBehavior.Cascade);
            builder.Entity<LandskapeModel>().HasMany(x => x.Reviews).WithOne(x => x.LandskapeModel).HasForeignKey(x => x.Id).OnDelete(DeleteBehavior.Cascade);
            builder.Entity<InstitutionModel>().HasMany(x => x.Reviews).WithOne(x => x.InstitutionMode).HasForeignKey(x => x.Id).OnDelete(DeleteBehavior.Cascade);

            builder.Entity<EventModel>().HasMany(x => x.Images).WithOne(x => x.EventModel).HasForeignKey(x => x.Id).OnDelete(DeleteBehavior.Cascade);
            builder.Entity<LandskapeModel>().HasMany(x => x.Images).WithOne(x => x.LandskapeModel).HasForeignKey(x => x.Id).OnDelete(DeleteBehavior.Cascade);
            builder.Entity<InstitutionModel>().HasMany(x => x.Images).WithOne(x => x.InstitutionModel).HasForeignKey(x => x.Id).OnDelete(DeleteBehavior.Cascade);

            //added
            builder.Entity<EventModel>().HasMany(x => x.Trips).WithMany(x => x.Events);
            builder.Entity<InstitutionModel>().HasMany(x => x.Trips).WithMany(x => x.Institutions);
            builder.Entity<LandskapeModel>().HasMany(x => x.Trips).WithMany(x => x.Landskapes);
